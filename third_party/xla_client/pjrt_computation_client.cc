...
PjRtComputationClient::PjRtComputationClient() {
  ...
  if (device_type == "CPU") {
    ...
    client = std::move(xla::GetCpuClient(/*asynchronous=*/false).ValueOrDie());
  } else if (device_type == "TPU") {
    TF_VLOG(1) << "Initializing PjRt TPU client...";
    client = xla::GetTpuClient(/*max_inflight_computations=*/1).ValueOrDie();
  } else {
...
std::vector<ComputationClient::DataPtr> PjRtComputationClient::TransferToServer(
  ...
  for (auto& tensor : tensors) {
    ...
    std::shared_ptr<xla::PjRtBuffer> buffer =
        client->BufferFromHostLiteral(literal, pjrt_device)
            .ValueOrDie();
